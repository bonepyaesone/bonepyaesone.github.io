...
var ParallelUniverse = function(docObj) 
{
	...
	function isMyanmar (obj)
	{
		...
		{
			if(reMyanmar[i].test(obj.textContent))
				return true;
		}
		return false;
	}
	...
	function isUnicode_my(obj)
	{
		...
		{
			if(reUnicode_my[i].test(obj.textContent))
				return true;
		}
		return;
	}
	...
	function isZawgyi(obj)
	{
		...
		{
			if(reZawgyi[i].test(obj.textContent))
				return true;
		}
		return false;
	}
	...
	function gmailInject()
	{
		...
		if(doc)
		{
			...
			doc.body.appendChild(doc.createElement('script')).textContent=code;
		}
		else
			return;
	}
